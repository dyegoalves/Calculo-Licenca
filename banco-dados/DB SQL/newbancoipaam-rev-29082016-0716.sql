...
CREATE TABLE IF NOT EXISTS `ipaam`.`empresas` (
  ...
  `numerodeempregados` VARCHAR(255) NOT NULL,
  `areautil` VARCHAR(255) NOT NULL,
  `atividades_id` INT NOT NULL,
  ...
  INDEX `fk_empresas_processos1_idx` (`processos_id` ASC),
  ...
  CONSTRAINT `fk_empresas_portes1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_empresas_processos1`
    FOREIGN KEY (`processos_id`)
    REFERENCES `ipaam`.`processos` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
